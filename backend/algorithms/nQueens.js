...
const nQueens = (n) => {
  ...
  // Solve the problem
  const result = solveNQueens(board, 0, steps, explanations);
...
const isSafe = (board, row, col, n, steps, explanations) => {
  const checkingPositions = [];
  
  // Check left side of current row
  for (let i = 0; i < col; i++) {
    checkingPositions.push([row, i]);
    if (board[row][i] === 1) return false;
  }

  // Check upper diagonal
  for (let i = row, j = col; i >= 0 && j >= 0; i--, j--) {
    checkingPositions.push([i, j]);
...
const solveNQueens = (board, col, steps, explanations) => {
  ...
  for (let row = 0; row < n; row++) {
    ...
    if (isSafe(board, row, col, n)) {
      ...
      if (solveNQueens(board, col + 1, steps, explanations)) {
        return true;
      }
